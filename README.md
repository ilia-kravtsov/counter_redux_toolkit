# Counter State Manager Playground (RTK version)

Данный проект является частью научно-исследовательской работы (НИР), 
посвящённой сравнительному анализу UI / Client State Managers в React-приложениях.

В рамках проекта реализуется одно и то же минимальное React-приложение — 
расширенный счётчик, — в котором слой управления состоянием заменяется 
различными стейт-менеджерами при сохранении неизменной UI-части.

Текущая версия проекта реализует управление состоянием с использованием 
Redux Toolkit (RTK) и служит референсной точкой для последующего сравнения 
с другими решениями:

Zustand, MobX, Jotai, Effector, Extended React State Model

## Цель

Цель проекта — практически сравнить архитектурные и прикладные характеристики различных state-менеджеров, включая:

количество и сложность кода

архитектурный бойлерплейт

ментальную модель

влияние на рендеринг

размер production-бандла

## Функциональность

Приложение реализует расширенный счётчик со следующим глобальным состоянием:

count: number
step: number
lastUpdatedBy: 'increment' | 'decrement' | 'reset'

Доступные действия:

увеличение значения счётчика (increment)

уменьшение значения (decrement)

сброс значения (reset)

изменение шага инкремента (step)

отображение информации о последнем действии

Функциональность намеренно минимальна, чтобы:

не усложнять код UI

сфокусироваться на сравнении state-менеджеров